...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-nav><button class=close aria-hidden=true ng-click=$dismiss()>×</button><div class=btn-group><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.prevImage()>‹ Προηγούμενο</a> <a ng-href={{Lightbox.imageUrl}} target=_blank class=\"btn btn-xs btn-default custom-lightb\" title=\"Open in new tab\">Άνοιγμα σε νέο παράθυρο</a> <a class=\"btn btn-xs btn-default\" ng-click=Lightbox.nextImage()>Επόμενο ›</a></div></div><br><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}}> <div ng-if=Lightbox.isVideo(Lightbox.image) class=\"embed-responsive embed-responsive-16by9\"><video ng-if=!Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} controls autoplay></video><embed-video ng-if=Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} iframe-id=lightbox-video class=embed-responsive-item><a ng-href={{Lightbox.imageUrl}}>Watch video</a></embed-video></div></div></div>"
  );
      ...
      if (!Lightbox.isVideo(image)) {
        ...
        ImageLoader.load(imageUrl).then(function () {
          ...
          success({
            ...
            'imageCaption': 'Η εικόνα δεν βρέθηκε'
          });
        });
      } else {
        success();
